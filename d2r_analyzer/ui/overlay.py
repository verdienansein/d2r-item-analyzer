...
class ItemOverlay:
    ...
    def _build_ui(self, win: tk.Tk, ev: EvaluationSchema) -> None:
        ...
        tk.Label(
            header, text=f"Grade: {ev.grade}",
            fg=grade_color, bg=BG, font=("Palatino Linotype", 18, "bold")
        ).pack(side="left", padx=(0, 14))

        tk.Label(
            header, text=ev.verdict.replace("_", " "),
            fg=verdict_color, bg=BG, font=FONT_TITLE
        ).pack(side="left")
        ...
        # ── Best build ─────────────────────────────────────
        if ev.best_build:
            row(win, "Best for:", ev.best_build, "#C8A96E", BG, FONT_LABEL, FONT_BODY, pad)
            _divider(win, DIVIDER)

        # ── Trade value + roll quality ──────────────────────
        row(win, "Trade value:", ev.trade_value.capitalize(), "#C8A96E", BG, FONT_LABEL, FONT_BODY, pad)
        row(win, "Roll quality:", ev.roll_quality.capitalize(), "#C8A96E", BG, FONT_LABEL, FONT_BODY, pad)
        _divider(win, DIVIDER)

        # ── Good affixes ───────────────────────────────────
        if ev.good_affixes:
            tk.Label(win, text="✔ Good affixes",
                     fg="#00FF00", bg=BG, font=FONT_LABEL, anchor="w"
            ).pack(fill="x", **pad)
            for affix in ev.good_affixes:
                tk.Label(win, text=f"  {affix}",
                         fg="#A8D8A8", bg=BG, font=FONT_BODY, anchor="w"
                ).pack(fill="x", padx=14)

        # ── Wasted slots ────────────────────────────────────
        if ev.wasted_slots:
            tk.Label(win, text="✘ Wasted slots",
                     fg="#FF4444", bg=BG, font=FONT_LABEL, anchor="w"
            ).pack(fill="x", **pad)
            for slot in ev.wasted_slots:
                tk.Label(win, text=f"  {slot}",
                         fg="#D08080", bg=BG, font=FONT_BODY, anchor="w"
                ).pack(fill="x", padx=14)

        _divider(win, DIVIDER)

        # ── Reasoning ──────────────────────────────────────
        tk.Label(win, text=ev.reasoning,
                 fg="#C8A96E", bg=BG, font=FONT_BODY,
                 wraplength=280, justify="left", anchor="w"
        ).pack(fill="x", padx=14, pady=(4, 10))

        # ── Dismiss hint ───────────────────────────────────
        tk.Label(win, text="ESC or click to dismiss",
                 fg="#5C3D11", bg=BG, font=("Palatino Linotype", 7)
        ).pack(pady=(0, 6))
